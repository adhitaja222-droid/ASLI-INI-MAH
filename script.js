        ...
        (function(){
          // ── Satu bin untuk likes DAN views ──
          var BIN_ID  = '6a083590c0954111d83006e3';
          ...
          // Simpan likes + views sekaligus ke 1 bin
          function saveToJsonBin(newLikes, currentViews) {
            return fetch(BASE, {
              method: 'PUT',
              headers: HEADERS,
              body: JSON.stringify({ likes: newLikes, views: currentViews })
            })
            .then(r => r.json())
          ...
          window.toggleProfileLike = function(){
            ...
            var newCount = willLike ? count + 1 : Math.max(0, count - 1);
            ...
            // Baca views terkini dulu, lalu simpan likes + views bareng
            fetch(BASE + '/latest', { headers: { 'X-Master-Key': API_KEY } })
              .then(r => r.json())
              .then(j => {
                var latestViews = j.record.views || 0;
                return saveToJsonBin(newCount, latestViews);
              })
...
/* ============================= */
  ...
  (function(){
    ...
    fetch(BASE + '/latest', { headers: { 'X-Master-Key': API_KEY } })
      ...
      .then(j => {
        let views = (j.record && j.record.views) ? j.record.views : 0;
        if (!sessionStorage.getItem('neysa_viewed')) {
          sessionStorage.setItem('neysa_viewed', '1');
          views++;
          // Hanya simpan field "views" — jangan campur dengan "likes" dari bin lain
          fetch(BASE, {
            method: 'PUT',
            headers: HEADERS,
      ...
      .catch(err => {
        console.warn('[ViewCounter] Error:', err);
